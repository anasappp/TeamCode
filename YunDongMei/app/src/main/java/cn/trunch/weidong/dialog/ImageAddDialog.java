...
ImageAddDialog extends BottomSheetDialog {
    ...
    private void uploadImage(final List<String> imagePaths) {
        for (int i = 0; i < imagePaths.size(); i++) {
            BmobFile bmobFile = new BmobFile(new File(imagePaths.get(i)));
            bmobFile.uploadblock(new UploadFileListener() {
                ...
                public void done(BmobException e) {
                    if(e==null){
                        ...
                        imageUris.clear();
                        onImageSelectListener.onSelect(bmobFile.getFileUrl(),dialog);
                    }else{
                        //toast("上传文件失败：" + e.getMessage());
                        onImageSelectListener.onFailure(dialog);
                    }
        ...
        /*new UploadApi(ApiUtil.MULTFILE_UPLOAD, imagePaths, ApiUtil.MULT_POST).uploads(new ApiListener() {
            ...
            public void success(Api api) {
                ...
                Gson gson = new Gson();
                List<String> imgs;
                imgs = gson.fromJson(u.getJsonData().toString(), new TypeToken<List<String>>() {}.getType());
    ...
    public interface OnImageSelectListener {
        void onSelect(String imageUrls,Dialog dialog);
